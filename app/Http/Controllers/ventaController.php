...
class ventaController extends Controller
{
     ...
     */
    public function create(ComprobanteService $comprobanteService, Request $request): View
    {
        ...
        if ($request->has('cotizacion_id')) {
            $cotizacion = Cotizacion::with('productos')->find($request->cotizacion_id);
        }
    ...
    public function store(StoreVentaRequest $request): RedirectResponse
    {
        ...
        try {
            ...
            // Update Cotizacion status if exists
            if ($request->filled('cotizacion_id')) {
                Cotizacion::where('id', $request->cotizacion_id)->update(['estado' => 2]); // 2: Aprobada
            }
