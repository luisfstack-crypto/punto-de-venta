...
<div class="container-fluid px-4 py-4">
    ...
    <form action="{{ route('productos.update', ['producto' => $producto]) }}" method="POST" enctype="multipart/form-data" id="form-producto">
        @method('PATCH')
        @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="pv-field @error('nombre') is-invalid @enderror">
                            ...
                            <input type="text" id="nombre" name="nombre"
                                   value="{{ old('nombre', $producto->nombre) }}" placeholder="Ej: Laptop Dell Inspiron">
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="pv-field">
                            ...
                            <textarea id="descripcion" name="descripcion" placeholder="Descripción breve del producto...">{{ old('descripcion', $producto->descripcion) }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="pv-field @error('presentacione_id') is-invalid @enderror">
                            ...
                            <select id="presentacione_id" name="presentacione_id">
                                ...
                                @foreach ($presentaciones as $pres)
                                    <option value="{{ $pres->id }}" {{ old('presentacione_id', $producto->presentacione_id) == $pres->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="pv-img-grid mb-3" id="existing-images">
                            @foreach($producto->imagenes as $img)
                            <div class="pv-img-thumb" data-id="{{ $img->id }}">
                                <img src="{{ $img->img_path }}" alt="">
                                <button type="button" class="pv-img-remove" onclick="removeExisting({{ $img->id }}, this)">×</button>
                            </div>
                            @endforeach
                        </div>
                        @endif
        ...
        <div class="pv-card">
            ...
            <div class="pv-card-body">
                ...
                {{-- Toggle facturable --}}
                <div class="pv-toggle-card {{ old('facturable', $producto->facturable) ? 'active' : '' }}" id="toggle-facturable-card">
                    <div>
                    ...
                    <div class="form-check form-switch mb-0">
                        <input class="form-check-input" type="checkbox" role="switch"
                               ...
                               {{ old('facturable', $producto->facturable) ? 'checked' : '' }}>
                    </div>
                </div>

                {{-- Sección SAT colapsable --}}
                <div id="seccion-sat" class="{{ old('facturable', $producto->facturable) ? 'show' : '' }}">
                        ...
                        <div class="col-md-5">
                            <div class="pv-field @error('clave_producto_sat') is-invalid @enderror">
                                ...
                                <input type="text" id="clave_producto_sat" name="clave_producto_sat"
                                       value="{{ old('clave_producto_sat', $producto->clave_producto_sat) }}"
                        ...
                        <div class="col-md-3">
                            <div class="pv-field">
                                ...
                                <input type="text" id="codigo_interno" name="codigo_interno"
                                       value="{{ old('codigo_interno', $producto->codigo_interno) }}" placeholder="Tu referencia interna">
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="pv-field @error('tasa_cuota') is-invalid @enderror">
                                ...
                                <select id="tasa_cuota" name="tasa_cuota">
                                    <option value="">— Seleccionar —</option>
                                    <option value="0.160000" {{ old('tasa_cuota', $producto->tasa_cuota)=='0.160000' ? 'selected' : '' }}>IVA 16%</option>
                                    <option value="0.080000" {{ old('tasa_cuota', $producto->tasa_cuota)=='0.080000' ? 'selected' : '' }}>IVA 8% (zona fronteriza)</option>
                                    <option value="0.000000" {{ old('tasa_cuota', $producto->tasa_cuota)=='0.000000' ? 'selected' : '' }}>Tasa 0%</option>
                                    <option value="Exento"   {{ old('tasa_cuota', $producto->tasa_cuota)=='Exento'   ? 'selected' : '' }}>Exento</option>
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="pv-field @error('unidad_medida') is-invalid @enderror">
                                ...
                                <select id="unidad_medida" name="unidad_medida">
                                    ...
                                    <option value="Kilogramo"  {{ old('unidad_medida', $producto->unidad_medida)=='Kilogramo'  ? 'selected' : '' }}>Kilogramo</option>
                                    ...
                                    <option value="Servicio"   {{ old('unidad_medida', $producto->unidad_medida)=='Servicio'   ? 'selected' : '' }}>Servicio</option>
                                    <option value="Hora"       {{ old('unidad_medida', $producto->unidad_medida)=='Hora'       ? 'selected' : '' }}>Hora</option>
                        ...
                        <div class="col-md-6">
                            <div class="pv-field @error('clave_unidad_sat') is-invalid @enderror">
                                ...
                                <select id="clave_unidad_sat" name="clave_unidad_sat">
                                    <option value="">— Seleccionar —</option>
                                    <option value="H87" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='H87' ? 'selected' : '' }}>H87 – Pieza</option>
                                    <option value="KGM" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='KGM' ? 'selected' : '' }}>KGM – Kilogramo</option>
                                    <option value="LTR" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='LTR' ? 'selected' : '' }}>LTR – Litro</option>
                                    <option value="MTR" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='MTR' ? 'selected' : '' }}>MTR – Metro</option>
                                    <option value="XBX" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='XBX' ? 'selected' : '' }}>XBX – Caja</option>
                                    <option value="E48" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='E48' ? 'selected' : '' }}>E48 – Servicio</option>
                                    <option value="HUR" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='HUR' ? 'selected' : '' }}>HUR – Hora</option>
                                    <option value="XPK" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='XPK' ? 'selected' : '' }}>XPK – Paquete</option>
                                    <option value="ACT" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='ACT' ? 'selected' : '' }}>ACT – Actividad</option>
                                    <option value="GRM" {{ old('clave_unidad_sat', $producto->clave_unidad_sat)=='GRM' ? 'selected' : '' }}>GRM – Gramo</option>
                                </select>
                            </div>
        ...
        <div class="pv-form-footer pv-card" style="margin-top:-1px; border-top: none;">
            ...
            <button type="submit" class="btn-pv-save">Actualizar producto</button>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    inputImg?.addEventListener('change', function () {
        ...
        const totalActual = document.querySelectorAll('#existing-images .pv-img-thumb').length;
        const disponibles = 5 - totalActual - selectedFiles.length;
    ...
    function removeExisting(id, btn) {
        btn.closest('.pv-img-thumb').remove();
        // Agregar un input hidden para notificar al servidor que debe borrarse (puedes implementarlo después)
    }
